...
const checkpointQuestions = [
  {
    question: "A message says your account closes in 10 minutes and gives a strange link. What should you do?",
    answers: [
      { text: "Click fast", correct: false },
      { text: "Check the real website first", correct: true }
    ],
    lesson: "Pressure is a common phishing trick. Open the real website yourself instead of trusting the link."
  },
  {
    question: "Which password is safer?",
    answers: [
      { text: "Sofia2024!", correct: false },
      { text: "River!Cloud92#Train", correct: true }
    ],
    lesson: "Long, mixed, less predictable passwords are harder to guess."
  },
  {
    question: "A website asks for your email before explaining why. Is that a good sign?",
    answers: [
      { text: "No, slow down", correct: true },
      { text: "Yes, type it", correct: false }
    ],
    lesson: "A trustworthy site should explain why it needs your data before asking for it."
  },
  {
    question: "What does two-factor login add?",
    answers: [
      { text: "A second proof step", correct: true },
      { text: "A shorter password", correct: false }
    ],
    lesson: "Two-factor login helps protect the account even if the password is stolen."
  },
  {
    question: "A download starts from a random pop-up. What is the safer choice?",
    answers: [
      { text: "Cancel it", correct: true },
      { text: "Open it quickly", correct: false }
    ],
    lesson: "Unexpected downloads can contain malware. Only download files from sources you trust."
  }
];
...
function showCheckpointQuestion() {
  ...
  const checkpointNumber = Math.floor(scoreValue / 1000) * 1000;
  ...
  gameCheckpointQuiz.querySelectorAll("button").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      nextCheckpointScore = checkpointNumber + 1000;
      hideCheckpointQuiz();
      clearObstacles();
...
function updateGame(timestamp) {
  ...
  gameSpeed = Math.min(12, 5 + scoreValue / 650);
  dinoScore.textContent = scoreValue;
  ...
  if (nextObstacleTime <= 0) {
    spawnObstacle();
    nextObstacleTime = Math.max(760, 1550 - scoreValue * 0.6) + Math.random() * 620;
  }

  nextPickupTime -= delta;
  if (nextPickupTime <= 0) {
    spawnPickup();
    nextPickupTime = Math.max(1250, 2600 - scoreValue * 0.35) + Math.random() * 1100;
  }

  gameObjects.forEach((gameObject) => {
    gameObject.x -= gameSpeed * (delta / 16);
    gameObject.element.style.transform = `translateX(${gameObject.x}px)`;
...
function startGame() {
  ...
  nextCheckpointScore = Math.floor(scoreValue / 1000) * 1000 + 1000;
  gameSpeed = 5;
  lastFrameTime = performance.now();
  nextObstacleTime = 850;
  nextPickupTime = 1300;
...
if (dinoGame) {
  ...
  dinoStartBtn.addEventListener("click", startGame);
  dinoRestartBtn.addEventListener("click", startGame);
