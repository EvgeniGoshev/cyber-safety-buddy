...
const obstacleTypes = [
  {
    label: "SCAM LINK",
    className: "link",
    lesson: "A strange link can lead to a fake website. Check the real domain before clicking."
  },
  {
    label: "FAKE LOGIN",
    className: "password",
  ...
  },
  {
    label: "123456",
    className: "password",
    lesson: "Weak passwords are easy to guess. Use long, unique passwords with different character types."
  }
];
...
function spawnObstacle() {
  ...
  obstacleElement.className = `scam-obstacle ${obstacleType.className}`;
...
function updateDinoPosition() {
  ...
  dinoPlayer.style.bottom = `${76 + dinoY}px`;
}
...
function updateGame(timestamp) {
  ...
  obstacles.forEach((obstacle) => {
    obstacle.x -= gameSpeed * (delta / 16);
    obstacle.element.style.transform = `translateX(${obstacle.x}px)`;

    if (didCollide(obstacle)) {
      endGame(obstacle.lesson);
    }
  });
...
function startGame() {
  ...
  dinoPlayer.style.bottom = "76px";
  dinoGame.classList.add("is-running");
  setGameMessage("", "", false);
  setGameLesson("Jump over scams and keep checking what tries to trick you.");
  dinoGame.focus();
...
window.addEventListener("beforeunload", () => {
  trackEvent("page_exit", { page: pageFromHash() });
});
